...
## Member auth (self-hosted, no third-party)

- Custom cookie-session auth — no Auth.js/NextAuth or external service. Stack: `lib/auth/password.ts` (Node `crypto.scryptSync` + `timingSafeEqual`), `lib/auth/session.ts` (HMAC-SHA256 signed `uid.role.exp` token, cookie `tambola_session`, httpOnly/lax, 7-day), `lib/auth/store.ts` (JSON file `data/members.json` + generated `data/secret`), `lib/auth/auth.ts` (DAL: `getCurrentUser`/`requireMember`/`requireAdmin`).
- Gating: server pages call `await requireMember("/game")` (redirects to `/login?next=…`). `app/game/page.tsx`, `app/tickets/page.tsx`, `app/admin/page.tsx` and `app/api/me/route.ts` all `export const dynamic = "force-dynamic"` (they read cookies). Server actions live in `app/actions/{auth,admin}.ts` ("use server").
...
- `data/` is gitignored (member store + session secret). First-run bootstrap creates the admin from `ADMIN_EMAIL`/`ADMIN_PASSWORD` (fallback: `admin@tambola.local` + a random password printed to stdout) and only if `data/members.json` is absent. Sessions invalidate if `data/` is deleted. `AUTH_SECRET` pins the signing key (default: generated `data/secret`); `COOKIE_SECURE=true` only behind HTTPS.
