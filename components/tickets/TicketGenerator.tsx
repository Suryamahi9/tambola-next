...
export default function TicketGenerator() {
  ...
  const generate = useCallback(() => {
    if (mode === "fullset") {
      const sets = Math.max(1, Math.min(5, count));
      const grids: Grid[] = [];
  ...
  const handleCountChange = (value: string) => {
    ...
    if (mode === "fullset") {
      setCount(Math.max(1, Math.min(5, Math.ceil(parsed / 6))));
    } else {
  ...
  const switchMode = (m: Mode) => {
    setMode(m);
    if (m === "fullset") {
      setCount((c) => Math.max(1, Math.min(5, Math.ceil(c / 6))));
    }
  };
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-6 dark:border-neutral-800 dark:bg-neutral-900">
        ...
        {mode === "fullset" && (
          <p className="mt-4 rounded-lg bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-800 dark:bg-amber-500/10 dark:text-amber-300">
            ...
            appears exactly once across the set. Choose 1–5 sets.
          </p>
        )}
      </div>
