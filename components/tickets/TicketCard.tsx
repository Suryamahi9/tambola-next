  ...
  return (
    <div className={`break-inside-avoid flex ${CARNIVAL_CARD} ${CARNIVAL_STAIN}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="relative z-[2] flex items-center justify-between py-1 pl-2 pr-3">
          <span className="font-vintage-display text-[11px] font-bold uppercase leading-none tracking-[0.22em] text-[#3a2b1c]">
            ✦ {brand}
          </span>
          <span className={`font-vintage-display text-[10px] font-bold italic tracking-wide ${a.text}`}>
...
function ThemedCard({ grid, name, index, total, called, style }: Props & { style: Exclude<TicketStyle, "carnival"> }) {
  ...
  const brand = name || "Tambola";
  const header = t.band ? (
    <div className={`flex items-center justify-between px-3 py-1.5 ${t.band}`}>
      <span className={`font-vintage-display text-[11px] font-bold uppercase leading-none tracking-[0.22em] ${t.brand}`}>
        ✦ {brand}
      </span>
      <span className={`font-vintage-display text-[10px] font-bold italic tracking-wide ${t.serial}`}>
        No. {String(index + 1).padStart(2, "0")}
  ...
  return (
    <div className={`break-inside-avoid overflow-hidden ${t.card}`}>
      {header}
